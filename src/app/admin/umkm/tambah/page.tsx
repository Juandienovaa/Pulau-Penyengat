import { Metadata } from "next";
import { UMKMForm } from "@/components/admin/UMKMForm";
import Link from "next/link";
import { ArrowLeft } from "lucide-react";

export const metadata: Metadata = {
  title: "Tambah UMKM | Admin",
};

export default function TambahUMKMPage() {
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <Link href="/admin/umkm" className="inline-flex items-center gap-2 text-sm font-bold text-navy/50 hover:text-navy transition-colors">
        <ArrowLeft className="w-4 h-4" /> Kembali ke Daftar UMKM
      </Link>
      
      <div>
        <h1 className="text-3xl font-serif font-bold text-navy mb-2">Tambah UMKM Baru</h1>
        <p className="text-navy/60 font-body">Masukkan detail UMKM untuk ditampilkan di direktori publik.</p>
      </div>

      <UMKMForm />
    </div>
  );
}
